...
- (id)initWithTitle:(NSString *)title
        ...
        contentView:(UIView *)contentView
         completion:(PXAlertViewCompletionBlock)completion
     allowsNoTitles:(BOOL)showNoTitles {
...
{
    self.visible = NO;
    [self dismissActivityIfContaintedInContentView];
    ...
    if (self.completion) {
        ...
        // test if no cancel button (sender == nil)
        
        if ((sender != nil) && (sender == self.cancelButton || sender == self.tap)) {
            cancelled = YES;
        }
...
- (void)dismissActivityIfContaintedInContentView {
    if ([self.contentView isKindOfClass:[PHIActivityIndicator class]]) {
        PHIActivityIndicator *activityIndicator = (PHIActivityIndicator*)self.contentView;
        [activityIndicator stopSpinning];
        [activityIndicator removeFromSuperview];
    }
}

- (void)updateActivityIfContainedInContentViewWithValue:(CGFloat)updatedProgressValue {
    if ([self.contentView isKindOfClass:[PHIActivityIndicator class]]) {
        PHIActivityIndicator *activityIndicator = (PHIActivityIndicator*)self.contentView;
        activityIndicator.progressValue = updatedProgressValue;
    }
}
...
+ (instancetype)showUpdateableActivityWithTitle:(NSString*)title
                                        message:(NSString *)message
                                    cancelTitle:(NSString *)cancelTitle
                          activityIndicatorType:(PHIActivityIndicatorType)indicatorType
                                     completion:(PXAlertViewCompletionBlock)completion {
    PHIActivityIndicator *activityIndicator = [[PHIActivityIndicator alloc] initWithIndicatorType:indicatorType];
    PXAlertView *alertView = [[self alloc] initWithTitle:title
                                                 message:message
                                             cancelTitle:cancelTitle
                                             otherTitles:nil
...
+ (instancetype)showActivityWithTitle:(NSString *)title
                          ...
                          cancelTitle:(NSString *)cancelTitle
                           completion:(PXAlertViewCompletionBlock)completion {
    return [self showActivityWithTitle:title
                               message:message
                           cancelTitle:cancelTitle
                 activityIndicatorType:PHISpinActivityIndicator
                            completion:completion];
}

- (void)dismissAlertViewWithActivity {
    [self dismissActivityIfContaintedInContentView];
    [self dismiss:nil animated:YES];
}

- (void)updateAlertViewActivityWithProgressValue:(CGFloat)progress {
    [self updateActivityIfContainedInContentViewWithValue:progress];
}
